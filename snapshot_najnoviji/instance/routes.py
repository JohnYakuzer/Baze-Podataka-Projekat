...
def change_password():
    ...
    # Ako nije poslana nova lozinka, samo potvrdi validnost korisnika
    if not nova_lozinka:
        return jsonify({"msg": "Korisnik potvrđen. Molimo unesite novu lozinku."}), 200

    # Ako je nova lozinka poslana, proveri da li je validna (npr. nije prazna)
    if len(nova_lozinka) < 6:  # ili neka druga validacija
...
def send_message():
    ...
    # Ovde ide slanje poruke – za sada samo print
...
def admin_letovi():
    if not current_user.has_role('admin'):
        return redirect(url_for('routes.index'))  # redirect na index u blueprintu routes
